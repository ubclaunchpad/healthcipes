...
DELIMITER $$
USE `umami_db`$$
CREATE PROCEDURE `createRecipe` (IN `_recipe_id` INT, IN `_name` VARCHAR(50), IN `_created_time` DATETIME, IN `_user_id` VARCHAR(50), IN `_protein` INT, IN `_carbs` INT, IN `_calories` INT, IN `_servings` INT, IN `_vegetarian` BOOLEAN, IN `_vegan` BOOLEAN, IN `_cooking_time` INT)
BEGIN
